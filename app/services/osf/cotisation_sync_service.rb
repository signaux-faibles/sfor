# app/services/osf/cotisation_sync_service.rb
# Service to synchronize cotisation data using PostgreSQL cursors for optimal performance

module Osf
  class CotisationSyncService < BaseOsfSyncService
    BATCH_SIZE = 1000
    ...
    def initialize(months_back: nil)
      ...
      @source_relation = "#{@schema}.clean_cotisation"
    end
    ...
    def sync_data
      ...
      @logger.info "Cotisation sync completed.
    ...
    def process_with_cursor(base_date_filter) # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
      cursor_name = "cotisation_cursor_#{Process.pid}_#{Time.current.to_i}"
    ...
    def process_cursor_batch(distant_records) # rubocop:disable Metrics/AbcSize
      ...
      if records_to_create.any?
        ActiveRecord::Base.transaction do
          OsfCotisation.insert_all(records_to_create)
          increment_stat(:created, records_to_create.size)
          @logger.debug "Bulk created #{records_to_create.size} cotisation records"
        end
      end
    ...
    def build_cotisation_attributes(distant_record)
      {
        siret: distant_record["siret"],
        periode: parse_date(distant_record["periode"]),
        du: safe_to_float(distant_record["du"])
      }
    end
  end
end
