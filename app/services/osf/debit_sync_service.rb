# app/services/osf/debit_sync_service.rb
...
module Osf
  class DebitSyncService < BaseOsfSyncService # rubocop:disable Metrics/ClassLength
    ...
    def initialize(months_back: nil)
      ...
      @source_relation = "#{@schema}.clean_debit"
    end
    ...
    def sync_data # rubocop:disable Metrics/MethodLength
      @logger.info "Starting debit data synchronization from #{@source_relation} using PostgreSQL cursor"

      @logger.info "Clearing existing osf_debits table"
      OsfDebit.delete_all
      ...
      @logger.info "Debit sync completed.
    ...
    def process_with_cursor(base_date_filter) # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
      cursor_name = "debit_cursor_#{Process.pid}_#{Time.current.to_i}"
    ...
    def process_cursor_batch(distant_records) # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
      ...
      if records_to_create.any?
        ActiveRecord::Base.transaction do
          OsfDebit.insert_all(records_to_create)
          increment_stat(:created, records_to_create.size)
          @logger.debug "Bulk created #{records_to_create.size} debit records"
        end
      end
    ...
    def build_debit_attributes(distant_record)
      {
