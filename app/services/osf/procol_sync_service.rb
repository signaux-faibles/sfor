# app/services/osf/procol_sync_service.rb
# Service to synchronize procol data using PostgreSQL cursors for optimal performance
    ...
    def process_cursor_batch(distant_records) # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
      ...
      if records_to_create.any?
        ActiveRecord::Base.transaction do
          OsfProcol.insert_all(records_to_create)
          increment_stat(:created, records_to_create.size)
          @logger.debug "Bulk created #{records_to_create.size} procol records"
        end
      end
