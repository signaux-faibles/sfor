...
export default class extends Controller {
  ...
  handleKeydown(event) {
    if (event.key === "ArrowDown") {
      ...
      const isOpen = this.isListboxDisplayed()
      if (isOpen) {
        this.clearResults()
      } else {
  ...
  clearOptionFocus() {
    this.getOptions().forEach(option => option.classList.remove("sf-multiselect-focused"))
  ...
  renderResults(options) {
    ...
    if (this.hasLabelValue) {
      this.resultsTarget.setAttribute("aria-label", this.labelValue)
    }

    if (options.length === 0) {
      this.resultsTarget.innerHTML = '<div class="fr-alert fr-alert--info fr-mt-2w" role="status"><p>Aucun résultat trouvé.</p></div>'
    ...
    options.forEach((opt, index) => {
      ...
      html += `<li id="${optionId}" class="sf-multiselect-result-item${selectedClass}" role="option" tabindex="-1" aria-checked="${opt.selected}"`
  ...
  clearResults() {
    if (this.resultsTarget) {
      ...
      this.resultsTarget.removeAttribute("aria-label")
    }
    this.focusedOptionIndex = -1
